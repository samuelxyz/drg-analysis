...
I have to start out with a confession: Conductive Thermals applies twice as fast when taking the tier 4 plasma splash upgrade: once from the direct hit and once from the splash. This is in no small part due to yours truly - during the experimental phase of season 5, I noticed that the temperature buff was applied by plasma splash but not the damage buff. So, I posted a bug report in the official experimental channels where I showed a clip of the drak hitting armor and applying only the temperature buff. GSG "fixed" this, so plasma splash AND the direct hit would each apply BOTH effects. So now, by spending one ammo you have the potential to apply up to two stacks of each effect. Which is quite relevant because time-to-kill improves dramatically, and the stacks expire after a limited time so applying twice as fast means you can get twice as many at maximum.
