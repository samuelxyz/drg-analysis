...
* In tier 4, either choice can work, but I prefer blowthrough. For normal M1000 builds we might care about the weakpoint upgrade reaching breakpoints, but we don't have focus shots here so it's not so much about the breakpoints as just general DPS. Blowthrough will apply the MFD effect on all the enemies it hits, though only the most recently hit one will be highlighted. Additionally, I think there's something to be said for the ability to hit a priority target despite it hiding behind another target. A relevant thought is to imagine taking weakpoint only for a single jelly to block your MFD focus shot and the jelly doesn't even die. (Lol)
