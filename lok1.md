...
* There is a complicated sequence of events that happens each time you fire a burst:
  * A ~0.22s delay before locks begin to be acquired
  ...
  * Finally, SMRT Trigger throws much of this out the window, letting you hold down the button and locking on much faster and ditching the 0.2s rate of fire delay and having an extremely high burst rate of fire. There are additional anomalies with it that cause it to just not match any kind of calculation I could put together. Therefore, SMRT Trigger is best analyzed by just recording the game and timing the recordings. Even ignoring the framerate variation.
...
Since SMRT and Exec are our top contenders, let's look at them in a little more detail. This time, I have specifically noted which bonuses apply. T5A electric dot is included. 
...
Note how T5A electricity loses a small bit of damage up front, butt ends up slightly ahead over time. When combined with the power of electric slow, I think this makes 11111 usually worth taking over 11112. The exception is if you anticipate having an abundant source of electricity elsewhere, because then 11112 gives a much bigger damage boost (green line).
...
When Exec isn't hitting weakpoints, it falls way behind. There are a few quite notable enemies where this is relevant, especially shellbacks. You can't one-mag a rolling Haz6p4 shellback with Exec unless you take 11112 and the shellback is on fire (rolling shellbacks are completely immune to electricity, and you aren't likely to hit a weakpoint). 
...
* **11122 SMRT**: A good pick. Damage, 4 locks, no electricity. This is the best of all Engi primaries against rolling shellbacks (which are immune to electricity).
...
* 11112 SMRT: Not great. Included here to show what happens when you take the 2-lock version of the gun. You can *sometimes* kill small targets more quickly, but the net rate of fire works out to be lower. On moving targets with heavy armor, you are also less likely to keep hitting the same armor plate that you broke with an earlier shot, which decreases damage output. 
  * If you take 11111 you can also milk the electric DOT's full value more easily to get a bit more ammo efficiency. However, your time to kill on small enemies will be a lot slower if you want to really use the electric damage, and the attention cost is much higher. I don't recommend this strategy.

## SMRT Breakpoint Samples

Breakpoints for Executioner are already given [on technicaldrg](https://www.reddit.com/r/technicaldrg/comments/wi63o3/lok1_executioner_breakpoints_haz6p4/) so I will put some SMRT breakpoints here as my side of the contribution.

The table below shows the number of shots taken to kill various bugs in combat tests (with bug AI enabled). Because Brundles and Shellbacks move a lot, their results are given as a range of what I experienced. For Brundles, I tried to consistently hit the same armor plate. For Shellbacks, I tried different situations of shooting them at long range vs in chaotic close quarters. Note that the curled up legs of a Shellback are completely invulnerable. Some of the results that took the most shots undoubtedly included shots getting blocked by this effect. Imagine them as a rolling car tire: aim for the tread, don't aim for the hub.
...
* Electricity is notably better against Brundles
* T5a electricity can often let you save one shot on small targets if you're willing to wait for DOT, but as I noted in the previous section I don't think this is really worth it
